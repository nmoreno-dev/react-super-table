...
import {Column, NormalizedColumn, SuperTableProps} from '../types/SuperTable.types';

export const useSuperTable = <T extends object>(
  data: T[],
  ...
  const [normalizedColumns, setNormalizedColums] = useState<NormalizedColumn<T>[]>([]);
  ...
  const handleSort = (columnToSort: NormalizedColumn<T>) => {
    ...
    const [selector, sortMethod] = [columnToSort.selector, columnToSort.sortMethod];
